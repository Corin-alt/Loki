...
@section('title') EC - Ajouter @endsection
            ...
            <div class="col-md-8">
                @if(Auth::check())
                    @if($user_log->role->rle_name == 'Responsable')
                        <div class="card">
                            <div class="card-header">{{ __('Ajouter un EC') }}</div>
                            <div class="card-body">
                                <form method="POST" action="{{ route('subjects.post') }}">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-6">
                                            <input id="sbj_name" type="text" class="form-control @error('sbj_name') is-invalid @enderror" name="sbj_name" value="{{ old('sbj_name') }}" required autocomplete="sbj_name" autofocus>
                                            @error('sbj_name')
                    ...
                    @endif
                @else
                    <div class="alert alert-danger text-center">Veuillez vous connecter pour ajouter un EC.</div>
                @endif
            </div>
        </div>
    </div>
